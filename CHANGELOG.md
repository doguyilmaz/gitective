...
- Signature shield in the hover (green verified, gray unverifiable, red bad) and author names linked to GitHub profiles when known.
- Hover action row is icons with tooltips plus `Open on GitHub`; colored files/insertions/deletions stat line; gear opens Whodunit settings; Whodunit settings also surface in a `git.blame` settings search.
